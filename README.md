...
`code_id` in every catalog, in eight steps. This is the case the opening
paragraph promises, and it is the base of every larger estate: compilers are
added to it afterwards, not set up differently. Run everything as root on that
...
**1. Install the package** as [above](#install).
...
[deploying.md](docs/deploying.md#where-your-control-repo-is-configured).
...
puppetserver](docs/commands.md#wiring-into-puppetserver), admitting this node by
...
[installation.md](docs/installation.md), which covers moving one compiler
before the rest.
...
[deploy-server.md](docs/deploy-server.md).
